...
public class CinemaVerifier {
    ...
    public static boolean isValidCinema(String name, String code, CinemaType cinemaType, SeatList seatList){
        ...
        if (isExistingCinema(code)) {
        	System.out.println("interfaces.Cinema already exists!");
            isValid = false; 
        }   
        if(isEmptyName(name)) {
        	System.out.println("interfaces.Cinema Name cannot be empty!");
        }
        if (!isValidCode(code)) {
        	System.out.println("interfaces.Cinema Code must be three characters long!");
        	isValid = false;
        }

        if (isEmptyCode(code)) {
        	System.out.println("interfaces.Cinema Code cannot be empty!");
        	isValid = false;
        }
