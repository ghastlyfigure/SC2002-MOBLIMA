...
public class CinemaVerifier {
    ...
    public static boolean isValidCinema(String name, String code, CinemaType cinemaType, SeatList seatList){
        ...
        if(isEmptyName(name)) {
        	System.out.println("Cinema Name cannot be empty!");
        }
        if (!isValidCode(code)) {
        	System.out.println("Cinema Code must be three characters long!");
        	isValid = false;
        }

        if (isEmptyCode(code)) {
        	System.out.println("Cinema Code cannot be empty!");
        	isValid = false;
        }
